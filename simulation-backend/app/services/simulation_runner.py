...
class SimulationRunState:
    ...
    # Per-platform independent rounds and simulation time (for dual-platform parallel display)
...
class SimulationRunner:
    ...
    Responsible for:
    1. Running OASIS simulation in background process
    2. Parsing run logs, recording each Agent's actions
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config not found. Please call /prepare API first")
        ...
        try:
            ...
            # Create main log file to avoid process blocking from stdout/stderr pipe buffer overflow
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes encoding issues when third-party libs (e.g., OASIS) read files without specifying encoding
            env = os.environ.copy()
            ...
            # Set working directory to simulation directory (databases and files are generated here)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms were running, both must complete
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking if its actions.jsonl file exists
        ...
        # Check which platforms are enabled (by checking file existence)
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer platform from record, otherwise use default platform
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for force restarting simulation)
        ...
        Note: Will not delete configuration files (simulation_config.json) or profile files
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there's anything to clean (avoid printing useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        Called at Flask application startup to ensure all simulation processes are cleaned up on server shutdown
        """
        ...
        # WERKZEUG_RUN_MAIN=true means this is the reloader subprocess
        # If not in debug mode, this env var doesn't exist, registration is also needed
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True means environment is alive, False means it has shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Get detailed status info for simulation environment

        Args:
            simulation_id: Simulation ID

        Returns:
            Status details dict containing status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms simultaneously, return integrated results
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment not running or shut down, cannot perform Interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: In dual-platform simulation, interview both platforms per Agent
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        """
        Close simulation environment (not stop the simulation process)
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (timed out waiting for response, environment may be shutting down)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            limit: Per-platform return count limit
        ...
        # If multiple platforms queried, limit total count
