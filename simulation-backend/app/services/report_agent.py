...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation completion (content only, does not indicate entire section is complete)"""
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs differ from agent_log.jsonl in that they are plain text format console output.
    """
...
[Return Content]
- Relevant factual source text (can be directly quoted)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Broad Search - Get Full Picture View]
...
[Use Cases]
- Need to understand event perspectives from different roles (What do students think? What does media say? What is the official response?)
...
PLAN_SYSTEM_PROMPT = """\
You are a "Future Prediction Report" writing expert with a "God's eye view" of the simulation world — you can observe every Agent's behavior, statements, and interactions in the simulation.

[Core Concept]
We have built a simulation world and injected specific "simulation requirements" as variables. The evolution results of the simulation world represent predictions of what may happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
- The report title should be bold, evocative, and journalistic — NOT generic academic phrasing
- Think magazine cover lines, not thesis titles. Use vivid language that captures the most dramatic or surprising prediction
- Bad: "Analysis of Universal Basic Income Impact on Consumer Spending Patterns"
- Good: "The $1,500 Experiment: How Free Money Rewired a Nation in Six Months"
- Good: "When Everyone Gets Paid: The Unexpected Winners and Losers of UBI"
- Section titles should follow the same principle — punchy, specific, intriguing
- The summary should read like a compelling one-sentence hook, not a dry abstract
...
3. [Language Consistency - Quoted Content Must Be in the Report Language]
   ...
   - All content in the report must be written in English
   - When quoting content returned by tools, translate it into fluent English before including it in the report
   - Maintain the original meaning during translation, ensuring natural and smooth expression
   - This rule applies to both body text and content within quote blocks (> format)
...
- Do NOT add a section main heading at the beginning of content
- Section titles are automatically added by the system; you only need to write plain body text
...
Weibo, as the first scene of public opinion, served as the core function for initial information release:
...
Each response you can only do one of the following two things (cannot do both simultaneously):
...
Strictly prohibited:
- Do not include both tool calls and Final Answer in a single response
...
- Do NOT write "{section_title}" as a heading
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Notice] You have only called tools {tool_calls_count} times, at least {min_tool_calls} times are required. "
...
REACT_UNUSED_TOOLS_HINT = "\nTip: You haven't used: {unused_list} yet. Consider trying different tools for multi-angle information"
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break out of loop to forced conclusion
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    # First two times: discard this response, ask LLM to respond again
                    messages.append({"role": "assistant", "content": response})
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] You included both a tool call and Final Answer in a single response, which is not allowed.\n"
                            ...
                            "Please respond again, doing only one of these things."
                        ),
                    })
                    continue
                else:
            ...
            # Directly adopt this content as the final answer, no more idle spinning
        ...
        # Check if LLM response is None during forced conclusion
        if response is None:
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Start reading from which line (for incremental retrieval, 0 means from the beginning)
    ...
    def save_section(
        ...
        # Build section Markdown content - clean up potentially duplicated headings
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        while cleaned_lines and cleaned_lines[0].strip() in ['---', '***', '___']:
            cleaned_lines.pop(0)
            # Also remove blank lines after separators
            while cleaned_lines and cleaned_lines[0].strip() == '':
                cleaned_lines.pop(0)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate heading (same content heading within previous 5 lines)
                is_duplicate = False
