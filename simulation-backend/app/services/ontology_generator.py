...
**Acceptable**:
- Specific individuals (public figures, parties involved, opinion leaders, expert scholars, ordinary people)
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person does not belong to any other more specific person type, they are classified here.
   - `Organization`: Fallback type for any organization. When an organization does not belong to any other more specific organization type, it is classified here.
...
- Various people appear in texts, such as "primary school teachers", "bystanders", "some netizen"
...
- Person: Any natural person (used when not fitting the specific types above)
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text is {original_length} characters in total; the first {self.MAX_TEXT_LENGTH_FOR_LLM} characters have been extracted for ontology analysis)..."
...
4. All entity types must be real-world agents that can speak; they cannot be abstract concepts
