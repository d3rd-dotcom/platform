...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to obtain rich information about an entity

        Zep does not have a built-in hybrid search API, so edges and nodes must be
        searched separately and then merged. Uses parallel requests for efficiency.
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, restoring real-world situations as closely as possible. You must return valid JSON format, and all string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, restoring real-world situations as closely as possible.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Personal memory (an important part of the persona, describing this individual's connection to events, and their existing actions and reactions in those events)
...
6. country: Country (in English, e.g., "China", "United States")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, restoring real-world situations as closely as possible.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   - Basic institutional information (official name, nature, founding background, main functions)
   ...
   - Institutional memory (an important part of the institutional persona, describing this institution's connection to events, and its existing actions and reactions in those events)
...
- Use English for all fields (gender field must be "other")
- age must be the integer 30, gender must be the string "other"
- Institutional account communications must align with its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Output the generated persona to console and log in real time
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Output the generated persona to console in real time (full content, no truncation)"""
        separator = "-" * 70
