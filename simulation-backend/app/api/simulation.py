...
# Adding this prefix prevents Agent from calling tools and replies with text directly
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview question by adding prefix to prevent Agent from calling tools
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, if not provided, obtained from project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - preparing: If config_generated=True, means completed
        # - running: Currently running, preparation was completed long ago
        # - completed: Run complete, preparation was completed long ago
        # - stopped: Stopped, preparation was completed long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.warning(f"Simulation {simulation_id} detection result: preparation incomplete (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparations to avoid redundant generation
    ...
    try:
        ...
        # Check if preparation is already complete (avoid redundant generation)
        if not force_regenerate:
            logger.debug(f"Checking simulation {simulation_id} preparation status...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"Check result: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"Simulation {simulation_id} preparation complete, skipping redundant generation")
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "ready",
                        "message": "Existing preparation found, no need for redundant generation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"Simulation {simulation_id} preparation incomplete, starting preparation task")
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Quick read entities (no edge info needed, just counting)
...
def get_prepare_status():
    ...
    try:
        ...
        # If simulation_id provided, first check if preparation is complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse reports directory, find reports matching simulation_id,
    if multiple found, return the latest (sorted by created_at)
    ...
    try:
        ...
        logger.warning(f"Finding simulation {simulation_id} 's report failed: {e}")
        return None
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich info including project name and description
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If the university releases...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check if currently generating (determined by state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time config (simulation duration, rounds, peak/valley periods)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config not found. Please call /prepare API first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Config file not found. Please call /prepare API first"
            }), 404
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (generic scripts in backend/scripts/)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it stops and cleans run logs first
    ...
    try:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running. Please call /stop API to stop it, or use force=true to force restart"
                            }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Graph memory update requires a valid graph_id. Please ensure the project has a built graph"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for frontend real-time dynamics display
    ...
    try:
        ...
        # recent_actions only shows the latest round content from both platforms
...
def interview_agent():
    ...
    Note: This function requires the simulation environment to be running (enters command waiting mode after completing simulation cycles)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or shut down. Please ensure simulation is complete and in command waiting mode."
            }), 400
...
def interview_agents_batch():
    ...
    Note: This function requires the simulation environment to be running
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // If platform not specified, use default
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // If not specified: dual-platform simulation interviews both platforms per Agent
    ...
    try:
        ...
        if env_alive and not use_interview_model:
            ...
            sim_dir = manager._get_simulation_dir(simulation_id)
            profiles = []
            for profile_file in ['reddit_profiles.json']:
                profile_path = os.path.join(sim_dir, profile_file)
            ...
            for interview in interviews:
                ...
                if agent_id < len(profiles):
                    ...
                    name = profile.get('name', profile.get('username', f'Agent {agent_id}'))

                    system_msg = f"""You are roleplaying as {name}. Stay fully in character based on this persona:

{persona}

Respond naturally as this person would. Be specific, opinionated, and authentic to their background, personality, and worldview. Keep your response concise (2-4 paragraphs)."""
...
def get_env_status():
    ...
    try:
        ...
        if env_alive:
            ...
            message = "Environment not running or shut down"
...
def close_simulation_env():
    ...
    Note: This differs from /stop API, which forcefully terminates the process,
    while this API lets the simulation gracefully close the environment and exit.
