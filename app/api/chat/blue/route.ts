...
- Never write app URL paths like /courses or /shadow-work. Refer to parts of MWA by name as normal things — "the course dashboard", "the course", "field notes", "quests" — never as links or slugs.
...
function describePage(pathname: string | null): string {
  ...
  const key = pathname.toLowerCase().replace(/\/+$/, '') || '/courses';
